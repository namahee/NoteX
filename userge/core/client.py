...
class Userge(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting NoteX Configs")
        kwargs = {
    ...
    async def start(self) -> None:
        ...
        _LOG.info(_LOG_STR, "Starting NoteX")
        await super().start()
        if self._bot is not None:
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        ...
        _LOG.info(_LOG_STR, "Stopping NoteX")
        await super().stop()
        _close_db()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(_sig: signal.Signals) -> None:
            ...
            _LOG.info(_LOG_STR, f"Received Stop Signal [{_sig.name}], Exiting NoteX ...")
            await _finalize()
        ...
        logbot.edit_last_msg("NoteX has started successfully!")
        logbot.end()
        ...
        try:
            if coro:
                ...
                _LOG.info(_LOG_STR, f"Idling NoteX - {mode}")
                idle()
