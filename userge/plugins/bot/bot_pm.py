...
if userge.has_bot:
    ...
    def drive_filter() -> filters:
        ...
        return filters.create(funcc, "DriveFilter")
    ...
    async def get_bot_infoo():
        ...
        if not (
            _CACHED_INFO
            and _CACHED_INFO.get("time")
            and (_CACHED_INFO.get("time") > datetime.timestamp(t_noww))
        ):
            try:
                ownerr_info = await userge.bot.get_user_dict(
                    Config.OWNER_ID[0], attr_dict=True
                )
            except (BadRequest, IndexError):
                _CACHED_INFO["owner"] = Config.OWNER_ID[0]
                ...
                _CACHED_INFO["owner"] = ownerr_info
            finally:
                _CACHED_INFO["bot"] = await userge.bot.get_user_dict(
                    "me", attr_dict=True
                )
                _CACHED_INFO["time"] = int(
                    datetime.timestamp(t_noww + timedelta(days=1))
                )
        return _CACHED_INFO
    ...
    async def check_new_bot_userr(user: Union[int, str, User]) -> bool:
        ...
        if userr_.id in Config.OWNER_ID:
            ...
            foundd = await BOT_DRIVE.find_one({"user_id": userr_.id})
            if not foundd:
    ...
    def default_owner_drive(from_userr):
        drive_msg = f"Hello Masterrrrrrrrrrrrrr **{from_userr.flname}** !\n"
        btnss = [
            [InlineKeyboardButton("➕  ADD TO GROUP", callback_data="add_to_grp")],
        ]
...
"""
    ...
    async def drive_bot(_, message: Message):
        ...
        if from_userr.id in Config.OWNER_ID:
            ...
            drive_msg = f"""
Aqui está o download! Apenas aperte no botão "LINK".
"""
            if Config.BOT_FORWARDS:
